/*
 * Suntikkan bobot hasil training Python (data/words_weights_v2.json) ke
 * arsitektur TFJS yang identik, verifikasi akurasinya pada set validasi
 * yang sama, lalu simpan ke public/models/bisindo-words/.
 */
...
const exp = JSON.parse(
  fs.readFileSync(path.join(root, "data", "words_weights_v2.json"), "utf8"),
);
const { words, weights, valAccuracy, valSigner } = exp;
console.log(`${words.length} kata, akurasi Python: ${(valAccuracy * 100).toFixed(2)}%`);
...
console.log("Bobot tersuntik");

// verifikasi pada set validasi yang sama dengan Python
const seqs = JSON.parse(
  fs.readFileSync(path.join(root, "data", "word_seqs.json"), "utf8"),
);
const labelWords = JSON.parse(
  fs.readFileSync(path.join(root, "data", "word_labels.json"), "utf8"),
);
...
const preds = tf.tidy(() =>
  model.predict(tf.tensor3d(xs)).argMax(-1).dataSync(),
);
let ok = 0;
...
if (Math.abs(acc - valAccuracy) > 0.02) {
  throw new Error("Akurasi TFJS menyimpang dari Python, batalkan");
}

fs.mkdirSync(outDir, { recursive: true });
fs.rmSync(path.join(outDir, "checkpoint"), { recursive: true, force: true });
await model.save(
